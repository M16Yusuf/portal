...
class ListBerita extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $query = $this->db->query("SELECT berita.id, berita.judul, berita.isi, berita.image, user.nama, berita.timestamp FROM berita INNER JOIN user ON berita.author=user.id ORDER BY berita.id DESC LIMIT $item OFFSET $start");
        ...
        if($this->input->post('keyword1')){
            ...
            $data['key'] = $cari;
            $query = $this->db->query("SELECT berita.id, berita.judul, berita.isi, berita.image, user.nama, berita.timestamp FROM berita INNER JOIN user ON berita.author=user.id WHERE berita.judul like '%$cari%' or berita.isi like '%$cari%' ORDER BY berita.id DESC;");
            $data['berita'] = $query->result_array();
